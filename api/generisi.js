export default async function handler(req, res) {
  ...
  try {
    ...
    let prompt;
    if (!jelo && sastojci) {
      prompt = `Na osnovu ovih sastojaka: "${sastojci}", odredi koje je jelo u pitanju i vrati JSON: {"detected_jelo":"IME JELA","opis":"Kratak opis jela (2-3 rečenice, maks 40 reči)"}. Vrsta restorana: ${restoran || 'tradicionalni restoran'}.`;
    } else {
    ...
    // Ako je magično prepoznavanje (bez naziva jela)
    if (!jelo && sastojci) {
      try {
